...
REPO_PATH = os.environ.get(
    "REPO_PATH",
    "/Users/alvinchen/Documents/develop/cursorAgent",
)
...
def check_agent_login():
    ...
    try:
        ...
        return ("logged in" in out) or ("authenticated" in out)
    except Exception:
        return False
...
def analyze_batch_failure(failed_commit, test_output):
    ...
    prompt = (
        "分析以下 Git commit 的改动，找出导致测试失败的原因:\n\n"
        "失败的 Commit: {commit}\n\n"
        "测试输出:\n"
        "{out}\n\n"
        "请简洁回答:\n"
        ...
        "2. 可能导致测试失败的原因 (1-2句话)\n"
        "3. 推荐修复方案\n"
    ).format(commit=failed_commit, out=test_output[:2000])
...
def run_batch_cherry_pick(commits, target_branch):
    ...
    cmd = (
        'bash "{script}" "{commits}" "{branch}" "{repo}" '
        '"python3 -m pytest tests/ -v 2>&1 || true"'
    ).format(
        script=script_path,
        commits=commits_str,
...
def extract_commits(output, prefix):
    ...
    if match:
        return [
            c.strip()
            for c in (match.group(1) or "").split(",")
            if c.strip()
        ]
    return []
...
def process_queue():
    ...
    while True:
        ...
        if result["success"]:
            ...
            ai_suggestion = analyze_conflict(
                conflict_files,
                result["output"],
            )
...
def handle_mention(event, say, logger):
    ...
    print("📩 收到消息 from {user}: {text}".format(user=user, text=text))

    # 提取命令
    clean_text = text
    if BOT_ID:
        clean_text = text.replace("<@{bid}>".format(bid=BOT_ID), "").strip()

    # 处理确认回复
    if ts in pending_confirm:
        confirm = (clean_text or "").lower().strip()
        if confirm in ("yes", "y"):
            info = pending_confirm[ts]
            say("✅ 好的，正在应用建议并重试...", thread_ts=ts)
            # TODO: 实现自动应用建议
    ...
    if ("!cherry-pick" in clean_text) or ("cherry-pick" in clean_text):
        ...
        if len(parts) >= 3:
            ...
            if queue_size > 1:
                msg = "⏳ 队列中 (前{}个任务)".format(queue_size - 1)
                say(msg, thread_ts=ts)
    ...
    if (
        ("!batch-cp" in clean_text)
        or ("batch-cp" in clean_text)
        or ("!step-cp" in clean_text)
        or ("step-cp" in clean_text)
    ):
        step_mode = ("!step-cp" in clean_text) or ("step-cp" in clean_text)
        tmp = (
            clean_text.replace("!batch-cp", "")
            .replace("batch-cp", "")
            .replace("!step-cp", "")
            .replace("step-cp", "")
            .strip()
        ...
        if len(parts) >= 2:
            ...
            say(
                "{mode} Cherry-Pick: {n} 个 commits → `{branch}`".format(
                    mode=mode,
                    n=len(commits),
                    branch=target_branch,
                ),
            ...
            if step_mode:
                ...
                if result.get("success"):
                    ...
                    ai_suggestion = analyze_batch_failure(
                    ...
                    if ai_suggestion:
                        msg2 = (
                            "🤖 **AI 分析:**\n\n"
                            "{suggest}"
                        ).format(suggest=ai_suggestion[:1500])
                        say(msg2, thread_ts=ts)
...
if __name__ == "__main__":
    ...
    print(
        "   Agent: {}".format(
            "已登录" if check_agent_login() else "未登录"
        )
    )
    print("=" * 50)
